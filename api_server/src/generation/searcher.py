...
async def _embed_sparse(query_text: str) -> SparseVector:
    def _embed():
        sparse_embedding = next(SPARSE_MODEL.embed([query_text]))
        return SparseVector(
...
async def _rerank_vectors(hits: object, query_text: str, level: str):
    # Check where the model is running
    # TODO: logger.info("The reranker model is running on: %s", RERANKER_MODEL.model.device)
...
async def _retreive_best_texto(client: AsyncQdrantClient, embedded_query: np.ndarray, query_text:str, cap_id: int):
    sparse_vector = _embed_sparse(query_text)
    ...
    # Rerank
    best_text_vectors = await _rerank_vectors(hits, query_text, "textos")
    return best_text_vectors

async def _retreive_best_capitulo(client: AsyncQdrantClient,embedded_query: np.ndarray, query_text:str, doc_id: str):
    sparse_vector = _embed_sparse(query_text)
    hits = client.query_points(
    ...
    # Rerank
    best_cap_ids = await _rerank_vectors(hits, query_text, "capitulos")
    best_cap_id = best_cap_ids[0].payload["cap_id"]
    return best_cap_id

async def _retreive_doc_id_from_topic(client: AsyncQdrantClient, topic: str):
    hits = client.scroll(
...
async def search(embedded_query: np.ndarray, query_text: str, topic: str) -> list[ScoredPoint]:
    # best_doc_id = self.retrieve_best_documento(embedded_query)
    client = QDRANT_CLIENT
    best_doc_id = await _retreive_doc_id_from_topic(client, topic)
    best_capitulo_id = await _retreive_best_capitulo(client, embedded_query, query_text, best_doc_id)
    best_texto_vectors = await _retreive_best_texto(client, embedded_query, query_text, best_capitulo_id)
